...
export default function TenantPaiementModule({ _tenantId, currentPlan = 'enterprise' }: TenantPaiementModuleProps) {
  useEffect(() => {
  ...
  const loadTenantData = () => {
    ...
    const abonnements = abonnementService.obtenirTousAbonnements(_tenantId);
    ...
    const allPaiements = abonnementService.obtenirPaiementsParAbonnement(currentSub?.abonnement_id || 0);
  ...
  const processPayment = async () => {
    ...
    try {
      ...
      const result = abonnementService.creerPaiement({
        tenant_id: tenantId,
      ...
      if (result.success) {
        ...
        if (couponCode) {
          ...
          if (coupon) {
            couponService.utiliserCoupon(coupon.coupon_id, _tenantId);
          }
        }
          ...
          <div className="w-full max-w-6xl max-h-[95vh] bg-white dark:bg-gray-800 rounded-xl shadow-2xl flex flex-col">
            ...
            <div className="p-4 sm:p-6 overflow-y-auto flex-1">
              <div className="grid grid-cols-1 gap-3 sm:gap-4 md:grid-cols-2 xl:grid-cols-3">
                {PLANS.map((plan) => (
                  <div key={plan.id} className={`relative rounded-lg border-2 p-3 sm:p-4 transition-all duration-300 bg-white dark:bg-gray-800 dark:border-gray-600 ${
                    plan.popular 
