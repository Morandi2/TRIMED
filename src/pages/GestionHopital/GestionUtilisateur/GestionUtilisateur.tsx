...
export const GestionUtilisateur: React.FC<GestionUtilisateurProps> = ({ _tenantId }) => {
  ...
  const loadData = () => {
    const data = utilisateurService.obtenirTousUtilisateurs(_tenantId);
    const statistics = utilisateurService.obtenirStatistiques(_tenantId);
  ...
  const handleSave = (formData: UtilisateurFormData) => {
    ...
    if (editingUtilisateur) {
      ...
      result = utilisateurService.creerUtilisateur(formData, _tenantId);
    }
