...
const GestionOrdonnances: React.FC = () => {
  ...
  const chargerOrdonnances = () => {
    const ordonnancesData = ordonnanceService.obtenirOrdonnancesParTenant(tenantConfig.tenant_id);
  ...
  const handleVoirOrdonnance = (ordonnanceId: number) => {
  ...
  const handleSupprimerOrdonnance = (ordonnanceId: number) => {
    const ordonnance = ordonnanceService.obtenirOrdonnance(ordonnanceId);
    if (ordonnance) {
      setSelectedOrdonnance(ordonnance);
      setModalType('delete');
  ...
  const handleDeleteConfirm = () => {
    if (selectedOrdonnance) {
      const success = ordonnanceService.supprimerOrdonnance(selectedOrdonnance.ordonnance_id);
      if (success) {
        chargerOrdonnances();
        setSuccessModal({
          isOpen: true,
  ...
  const handleSaveOrdonnance = (ordonnance: Ordonnance) => {
    chargerOrdonnances();
    setSuccessModal({
      isOpen: true,
      title: 'Opération réussie',
      message: 'L\'ordonnance a été sauvegardée avec succès.',
      type: 'success'
    });
  };
  ...
  const filteredOrdonnances = ordonnances.filter(ordonnance => {
    const matchesSearch = 
      ordonnanceService.obtenirNomPatient(ordonnance.patient_id).toLowerCase().includes(searchTerm.toLowerCase()) ||
      ordonnanceService.obtenirNomMedecin(ordonnance.medecin_id).toLowerCase().includes(searchTerm.toLowerCase()) ||
      ordonnance.validite.toLowerCase().includes(searchTerm.toLowerCase());
          ...
          <div className="relative bg-white dark:bg-gray-800 rounded-lg w-full max-w-2xl max-h-[90vh] overflow-hidden shadow-2xl">
            <div className="p-6 border-b border-gray-200 dark:border-gray-700">
              <div className="flex justify-between items-center">
                <h2 className="text-xl font-bold text-gray-900 dark:text-white">Détails de l'Ordonnance</h2>
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => handlePrintOrdonnance(selectedOrdonnance)}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-gray-900 dark:text-white">
                    {new Date(selectedOrdonnance.date_ordonnance).toLocaleDateString('fr-FR', {
                      year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit'
                    })}
                  </p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-600 dark:text-gray-400">Validité:</label>
                  <p className="text-gray-900 dark:text-white">{selectedOrdonnance.validite}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-600 dark:text-gray-400">Consultation:</label>
                  <p className="text-gray-900 dark:text-white">{ordonnanceService.obtenirConsultationInfo(selectedOrdonnance.consultation_id)}</p>
                </div>
                {selectedOrdonnance.prescriptions && selectedOrdonnance.prescriptions.length > 0 && (
                  <div>
                    ...
                    <div className="mt-2 space-y-2">
                      {selectedOrdonnance.prescriptions.map((prescription, index) => (
                ...
                {selectedOrdonnance.recommandations && (
                  <div>
                    <label className="text-sm font-medium text-gray-600 dark:text-gray-400">Recommandations:</label>
                    <p className="text-gray-900 dark:text-white whitespace-pre-wrap">{selectedOrdonnance.recommandations}</p>
                  </div>
                )}
              </div>
            </div>
