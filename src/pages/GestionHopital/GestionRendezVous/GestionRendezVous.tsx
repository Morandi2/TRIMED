...
export const GestionRendezVous: React.FC<GestionRendezVousProps> = ({ _tenantId }) => {
  ...
  useEffect(() => {
    loadData();
    setTypes(rendezVousService.obtenirTypes(_tenantId));
    setStatuts(rendezVousService.obtenirStatuts(_tenantId));
  }, [tenantId]);

  const loadData = () => {
    const data = rendezVousService.obtenirTousRendezVous(_tenantId);
    const statistics = rendezVousService.obtenirStatistiques(_tenantId);
    setRendezVous(data);
    setStats(statistics);
  };
  ...
  const handleSave = (formData: RendezVousFormData) => {
    ...
    if (editingRendezVous) {
      ...
      result = rendezVousService.creerRendezVous(formData, _tenantId);
    }
