...
export const ConsultationStats: React.FC<ConsultationStatsProps> = ({ consultations }) => {
  ...
  const stats = {
    ...
    aujourdhui: consultations.filter(c => new Date(c.date_consultation) >= startOfDay).length,
    semaine: consultations.filter(c => new Date(c.date_consultation) >= startOfWeek).length,
    mois: consultations.filter(c => new Date(c.date_consultation) >= startOfMonth).length
  };
