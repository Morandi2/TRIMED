...
export const MouvementModal: React.FC<MouvementModalProps> = ({
  ...
  const [formData, setFormData] = useState<MouvementFormData>({
    ...
    cout_unitaire: type === 'Entrée' ? medicament.prix_achat : undefined,
    destination: type === 'Sortie' ? 'Pharmacie interne' : '',
    fournisseur: type === 'Entrée' ? '' : '',
    numero_lot: medicament.lot_number || '',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      const result = medicamentService.creerMouvementStock(formData, _tenantId);
  ...
  return (
    <div className="fixed inset-0 z-[99999] flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-lg w-full max-w-2xl max-h-[90vh] overflow-hidden shadow-2xl transform transition-all duration-300 scale-100 z-[100000]">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className={`text-sm font-medium ${
                    calculateStockApres() < 0 ? 'text-red-600' : 'text-gray-800 dark:text-white/90'
                  }`}>
                    {calculateStockApres()} {medicament.unite_stock}
                  </p>
                </div>
              </div>
            </div>
