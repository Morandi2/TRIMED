...
export default function PaymentModule({ adminId, currentPlan = 'basic' }: PaymentModuleProps) {
 ...
 <div
 key={plan.id}
 className={`rounded-lg border p-6 transition-all hover:shadow-lg ${plan.isPopular
 ...
 className={`w-full rounded-lg px-4 py-2 text-sm font-medium transition-colors ${plan.isPopular
 ...
 {/* Modal pour choisir la méthode de paiement */}
...
// Composant pour la sélection du plan
function PlanSelectionModal({
 ...
 <div
 key={plan.id}
 className={`rounded-lg border p-6 transition-all hover:scale-105 ${plan.isPopular
 ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20'
 : 'border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800'
 ...
 className={`w-full rounded-lg px-4 py-2 text-sm font-medium transition-colors ${currentPlan === plan.id
...
// Composant pour choisir la méthode de paiement
...
function PaymentHistoryModal({
 ...
 getStatusColor: (status: string) => "success" | "warning" | "error" | "primary" | "info" | "light" | "dark" | undefined;
